...
-
        ...
        <div class="col-sm-12 col-md-12">
          <div class="title m-b-2"><span style="font-size:36px; color:#F00; font-style:italic; font-weight:bolder;">Order History</span></div>
          <div class="row">
            <div class="col-xs-12">
              <div class="table-responsive">
                <table class="table">
                  <thead>
                   ...
                   <tr>
                      <td style='font-size:24px;'>Order Id</td>
                      <td style='font-size:24px;'>Product</td>
                      <td style='font-size:24px;'>Date</td>
                      <td class="text-center" style='font-size:24px;'>Status</td>
                      <td class="text-center" style='font-size:24px;'>Invoice</td>
                    </tr>
                  </thead>
                  <tbody>
                  <?php
$select_client_cart_ship="SELECT * FROM cart inner join product on cart.pid=product.pid where clint_id='$clint' ";
$query_client_cart_ship=mysqli_query($conn,$select_client_cart_ship)or die(mysqli_error());
$num_client_cart_ship=mysqli_num_rows($query_client_cart_ship);

while($fetch_client_cart_ship=mysqli_fetch_array($query_client_cart_ship))
{
?> 
                    <tr>
                      <td >
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="badge" >
                      <?php 
					  if($num_client_cart_ship > 0 )
					  {
					  echo $fetch_client_cart_ship['crt_id']; 
					  }
					  else
					  {
						  echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					...
					echo $fetch_client_cart_ship['pname'];
				...
				?> 
					  ...
					  if($num_client_cart_ship > 0 )
					  {
					  echo $fetch_client_cart_ship['crt_date']; 
					  }
					  else
					  {
						  echo "DD MM YYYY";
                      ...
                      if($fetch_client_cart_ship['crt_status'] == 'Pending')
                      {
                      ?>
					  <span class="label label-warning" style="font-size:16px; background-color:#CCCC00; ">Pending</span>
                      <?php
                      }
                      else if($fetch_client_cart_ship['crt_status'] == 'Confirm')
                      {
                      ?>
					  <span class="label label-info" style="font-size:16px; background-color:#0099FF; ">Confirm</span>
                      <?php
                      }
                      else if($fetch_client_cart_ship['crt_status'] == 'Delivering')
                      {
                      ?>
					  <span class="label label-primary" style="font-size:16px; background-color:#FF9900; ">Delivering</span>
                      <?php
                      }
                      
					  else if($fetch_client_cart_ship['crt_status'] == 'Delivered')
                      {
                      ?>
					  <span class="label label-success" style="font-size:16px;background-color:#009900;">Delivered</span>
                      <?php
                      }
                      
					  else if($fetch_client_cart_ship['crt_status'] == 'Cancel')
                      {
                      ?>
					  <span class="label label-danger" style="font-size:16px;  background-color:#FF0000; ">Cancel</span>
                      <?php
                      }
                      else
					  {
                      ?>
					  <span class="label label-default" style="font-size:16px; background-color:#000; ">No Order Yet</span>
                      <?php
                      }
                      ?>
                      </td>
        <?php
        if ($fetch_client_cart_ship['crt_status'] == 'Delivering' || $fetch_client_cart_ship['crt_status'] == 'Delivered') {
        
        ?>
                      <td style="vertical-align:middle; text-align:center; ">
                      	
    <button><a  class="btn btn-medium" style="border-radius:10px; color:#09F; border:none; background-color:transparent;  margin: 0px; padding: 0px; font-size:20px; color:red;" href="invoice-db.php?pid=<?php echo $fetch_client_cart_ship['pid'];?>;" target="_blank"><i class="fa fa-print" style="font-size:25px; color:red; "></i>Print</a></button>
				  </td>
				  <?php
						}
						?>
