...
<form class="form">
  <div class="amount">
    <label class="amount-label">Amount:<br></label>
    <label name="amt" id="amt">₹<?php echo $grand_total; ?></label>
  </div>
  <label for="name" class="label">Name:</label>
  <input type="text" id="name" name="name" class="input-text">
  <label for="contact_name" class="label">Contact No:</label>
  <input type="text" id="contact_name" name="contact_name" class="input-text">
  <label for="email" class="label">Email:</label>
  <input type="text" id="email" name="email" class="input-text">
  <input type="button" value="Proceed to pay" id="place_order" name="place_order" class="sub" onclick="payNow()">
</form>
...
function payNow() {
  ...
  var amount = document.getElementById("amt").innerHTML.replace("₹", "").trim();
  ...
  jQuery.ajax({
    ...
    data: "amount=" + amount + "&name=" + name + "&contact_name=" + contact_name + "&email=" + email,
    success: function(result) {
      var options = {
        "key": "rzp_test_q96l6gy7r1CJeP", 
        "amount": amount * 100,
        "currency": "INR",
        "name": "GMART GROCERY",
        "description": "Transaction",
        "image": "https://dypdvfcjkqkg2.cloudfront.net/large/3637713-247.png",
